...
IF OBJECT_ID('tempdb..#EncounterSmartDataElement ') IS NOT NULL
DROP TABLE #EncounterSmartDataElement

IF OBJECT_ID('tempdb..#ScheduledAppointmentInfectionStatus ') IS NOT NULL
DROP TABLE #ScheduledAppointmentInfectionStatus

IF OBJECT_ID('tempdb..#ScheduledAppointment ') IS NOT NULL
DROP TABLE #ScheduledAppointment

IF OBJECT_ID('tempdb..#IEV ') IS NOT NULL
DROP TABLE #IEV
...
,'604'
  ...
  SELECT
   ...
   --,ISNULL(CONVERT(VARCHAR(1200),LEFT([INFECTION_NAMES],LEN([INFECTION_NAMES])-1)),'')			AS [INFECTION_STATUS]
   ,CASE
   ...
   --,ISNULL(CONVERT(VARCHAR(1200),LEFT([APPT_NOTES],LEN([APPT_NOTES])-1)),'')						AS [APPT_NOTES]
   ,CASE
   ...
   --,ISNULL(CONVERT(VARCHAR(1200),LEFT([SDE_VALUES],LEN([SDE_VALUES])-1)),'')						AS [VISIT_FLOW_COMMENTS]
   ,CASE
