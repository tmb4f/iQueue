...
MODS:     11/13/2018--TMB-- Create new stored procedure
          04/12/2019--TMB-- Add DEPT_SPECIALTY_NAME, PAT_MRN_ID, APPT_NOTE
		  05/15/2019--TMB-- Link following Infusion visits (10210004)
*****************************************************************************************************************************************/
...
DROP TABLE #RptgTemp
  ...
  SELECT
    ...
    --,LEAD(pa.[PAT_ENC_CSN_ID]) OVER (PARTITION BY pa.PAT_ID, CAST(pa.[APPT_DTTM] AS DATE) ORDER BY pa.[APPT_DTTM], pa.PAT_ENC_CSN_ID) AS [Next PAT_ENC_CSN_ID_unhashed]
	,HASHBYTES('SHA2_256',CAST(enc.PAT_ENC_CSN_ID AS VARCHAR(18))) AS [PAT_ENC_CSN_ID]
    --,LEAD(HASHBYTES('SHA2_256',CAST(enc.PAT_ENC_CSN_ID AS VARCHAR(18)))) OVER (PARTITION BY pa.PAT_ID, CAST(pa.[APPT_DTTM] AS DATE) ORDER BY pa.[APPT_DTTM], pa.PAT_ENC_CSN_ID) AS [Next PAT_ENC_CSN_ID]
    ...
    --,LEAD(pa.[DEPARTMENT_NAME]) OVER (PARTITION BY pa.PAT_ID, CAST(pa.[APPT_DTTM] AS DATE) ORDER BY pa.[APPT_DTTM], pa.PAT_ENC_CSN_ID) AS [Next DEPARTMENT_NAME]
    ...
    --,LEAD(pa.[APPT_STATUS_NAME]) OVER (PARTITION BY pa.PAT_ID, CAST(pa.[APPT_DTTM] AS DATE) ORDER BY pa.[APPT_DTTM], pa.PAT_ENC_CSN_ID) AS [Next APPT_STATUS_NAME]
  ...
  --     , APPT_DTTM
	 ...
	 --  , APPT_STATUS_C
	 --  , APPT_STATUS_NAME
	 --  --, [Next PAT_ENC_CSN_ID_unhashed]
	 --  --, [Next PAT_ENC_CSN_ID]
	 --  , [Next DEPARTMENT_ID]
	 --  --, [Next DEPARTMENT_NAME]
	 --  , [Next APPT_STATUS_C]
	 --  --, [Next APPT_STATUS_NAME]
  --FROM #ScheduledAppointment
  ----WHERE PAT_ID = 'Z101808'
  --ORDER BY PAT_ID, APPT_DTTM, SeqNbr

  --SELECT COUNT(*)
  --FROM #ScheduledAppointment pa
  --WHERE (pa.DEPARTMENT_ID IN (10210002 -- ECCC HEM ONC WEST
  --                           ,10210030 -- ECCC NEURO WEST
		--					 ,10243003 -- UVHE DIGESTIVE HEALTH
		--					 ,10243087 -- UVHE SURG DIGESTIVE HL
  ...
  --     , APPT_DTTM
	 ...
	 --  , APPT_STATUS_C
	 --  , APPT_STATUS_NAME
  --FROM #ScheduledAppointmentPlus
  ----WHERE PAT_ID = 'Z101808'
  --ORDER BY PAT_ID, APPT_DTTM, RecordId, [Next RecordId]
  ...
  --SELECT *
  --FROM #ScheduledAppointmentLinked
  --WHERE PAT_ID = 'Z101808'
  --ORDER BY PAT_ID, APPT_DTTM
 ...
 -- SELECT appt.*, notes.LINE, notes.CONTACT_DATE, notes.APPT_NOTE
 -- FROM #ScheduledClinicAppointmentDetail appt
 -- LEFT OUTER JOIN
 -- (
	...
	--WHERE LEN(APPT_NOTE) > 0) notes
 -- ON notes.PAT_ENC_CSN_ID = appt.PAT_ENC_CSN_ID_unhashed
 ...
 --        , appt.PAT_ENC_CSN_ID
  ...
  --INSERT INTO CLARITY_App.Stage.iQueue_Outpatient_Extract
  ...
  SELECT
   ...
   ,ISNULL(CONVERT(VARCHAR(254),LEFT([APPT_NOTES],LEN([APPT_NOTES])-1)),'')						AS [APPT_NOTE]
